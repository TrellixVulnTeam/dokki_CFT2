...
def create_data_lists(ICDAR_path, output_folder):
    ...
    # Training data
    train_path=os.path.join(ICDAR_path,"train")
    print(len(listdir(train_path)))

    # Find IDs of images in training data

    ids = [f.replace(".jpg","") for f in listdir(train_path) if isfile(join(train_path, f)) and f.endswith("jpg") ]

    for id in ids:
        # Parse annotation's txt file
        if not os.path.exists(os.path.join(train_path,  id + '.txt')):
            continue
        objects = parse_annotation(
            os.path.join(train_path,  id + '.txt'))
        if len(objects) == 0:
            continue
